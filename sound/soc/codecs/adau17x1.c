...
static const struct snd_kcontrol_new adau17x1_controls[] = {
	SOC_DOUBLE_R_TLV("Digital Capture Volume",
		ADAU17X1_LEFT_INPUT_DIGITAL_VOL, ADAU17X1_RIGHT_INPUT_DIGITAL_VOL,
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	ret = snd_soc_bulk_write_raw(codec, ADAU17X1_PLL_CONTROL,
			adau->pll_regs, 6);
...
static const struct snd_soc_dapm_widget adau17x1_dapm_widgets[] = {
	SND_SOC_DAPM_SUPPLY_S("PLL", 0, SND_SOC_NOPM, 0, 0, adau17x1_pll_event,
	...
	SND_SOC_DAPM_ADC("Left ADC", NULL, ADAU17X1_ADC_CONTROL, 0, 0),
	SND_SOC_DAPM_ADC("Right ADC", NULL, ADAU17X1_ADC_CONTROL, 1, 0),

	SND_SOC_DAPM_AIF_OUT("AIFOUT", "Capture", 0, SND_SOC_NOPM, 0, 0),
	SND_SOC_DAPM_AIF_IN("AIFIN", "Playback", 0, SND_SOC_NOPM, 0, 0),
};

static int adau17x1_check_sysclk_pll(struct snd_soc_dapm_widget *source,
	struct snd_soc_dapm_widget *sink)
{
	struct adau *adau = snd_soc_codec_get_drvdata(source->codec);

	return adau->clk_src == ADAU17X1_CLK_SRC_PLL;
}

static const struct snd_soc_dapm_route adau17x1_dapm_routes[] = {
	{ "SYSCLK", NULL, "PLL", adau17x1_check_sysclk_pll },
	...
	struct snd_soc_dapm_widget *sink)
{
	...
	return !adau->dsp_capture_bypass;
}

static int adau17x1_check_dsp_playback(struct snd_soc_dapm_widget *source,
	struct snd_soc_dapm_widget *sink)
{
	struct adau *adau = snd_soc_codec_get_drvdata(source->codec);

	return !adau->dsp_playback_bypass;
}

/* This is not exacly by the book. It would be better to provide MIXER or MUX
 * widgets for the DSP capture and playback path.  */
static const struct snd_soc_dapm_widget adau17x1_dsp_dapm_widgets[] = {
	SND_SOC_DAPM_SUPPLY("DSP", ADAU17X1_DSP_RUN, 0, 0, NULL, 0),
};
...
static int adau17x1_dsp_bypass_put(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	...
	bool bypass = ucontrol->value.integer.value[0];
	unsigned int tdm_slot;

	switch (mc->reg) {
	case ADAU17X1_SERIAL_INPUT_ROUTE:
		adau->dsp_playback_bypass = bypass;
		tdm_slot = adau->tdm_dac_slot;
		break;
	case ADAU17X1_SERIAL_OUTPUT_ROUTE:
		adau->dsp_capture_bypass = bypass;
		tdm_slot = adau->tdm_adc_slot;
		break;
	default:
		return -EINVAL;
	}
	...
	if (bypass)
		snd_soc_write(codec, mc->reg, (tdm_slot * 2) + 1);
	else
		snd_soc_write(codec, mc->reg, 0x00);

	return 0;
}

static int adau17x1_dsp_bypass_get(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_kcontrol_chip(kcontrol);
	struct adau *adau = snd_soc_codec_get_drvdata(codec);
	...
	switch (mc->reg) {
	case ADAU17X1_SERIAL_INPUT_ROUTE:
		bypass = adau->dsp_playback_bypass;
		break;
	case ADAU17X1_SERIAL_OUTPUT_ROUTE:
		bypass = adau->dsp_capture_bypass;
		break;
	default:
		return -EINVAL;
	}

	ucontrol->value.integer.value[0] = bypass;

	return 0;
}

static const struct snd_kcontrol_new adau17x1_dsp_controls[] = {
	SOC_SINGLE_EXT("DSP Bypass Playback Switch", ADAU17X1_SERIAL_INPUT_ROUTE,
		0, 1, 0, adau17x1_dsp_bypass_get, adau17x1_dsp_bypass_put),
	SOC_SINGLE_EXT("DSP Bypass Capture Switch", ADAU17X1_SERIAL_OUTPUT_ROUTE,
		0, 1, 0, adau17x1_dsp_bypass_get, adau17x1_dsp_bypass_put),
};
	...
	struct snd_pcm_hw_params *params, struct snd_soc_dai *dai)
{
	...
	snd_soc_update_bits(codec, ADAU17X1_CONVERTER0, 7, div);
	if (adau17x1_has_dsp(adau)) {
		snd_soc_write(codec, ADAU17X1_SERIAL_SAMPLING_RATE, div);
		snd_soc_write(codec, ADAU17X1_DSP_SAMPLING_RATE, dsp_div);
	}
	...
	return snd_soc_update_bits(codec, ADAU17X1_SERIAL_PORT1,
...
static int adau17x1_set_dai_clkdiv(struct snd_soc_dai *dai, int div_id, int div)
{
	...
	return snd_soc_update_bits(dai->codec, ADAU17X1_CLOCK_CONTROL,
		...
		unsigned int fmt)
{
	...
	snd_soc_write(codec, ADAU17X1_SERIAL_PORT0, ctrl0);
	snd_soc_write(codec, ADAU17X1_SERIAL_PORT1, ctrl1);
...
printk ("txmask %d rxmask %d slots %d width %d\n",tx_mask,rx_mask,slots,slot_width);
	...
	snd_soc_update_bits(codec, ADAU17X1_CONVERTER0,
		ADAU17X1_CONVERTER0_DAC_PAIR_MASK, conv_ctrl0);
	snd_soc_update_bits(codec, ADAU17X1_CONVERTER1,
		ADAU17X1_CONVERTER1_ADC_PAIR_MASK, conv_ctrl1);
	snd_soc_update_bits(codec, ADAU17X1_SERIAL_PORT0,
		ADAU17X1_SERIAL_PORT0_TDM_MASK, ser_ctrl0);
	snd_soc_update_bits(codec, ADAU17X1_SERIAL_PORT1,
		ADAU17X1_SERIAL_PORT1_BCLK_MASK, ser_ctrl1);


	if (adau17x1_has_dsp(adau)) {
		if (adau->dsp_playback_bypass) {
			snd_soc_write(codec, ADAU17X1_SERIAL_INPUT_ROUTE,
				(adau->tdm_dac_slot * 2) + 1);
		}
		if (adau->dsp_capture_bypass) {
			snd_soc_write(codec, ADAU17X1_SERIAL_OUTPUT_ROUTE,
	...
	enum adau17x1_micbias_voltage micbias)
{
	...
	snd_soc_write(codec, ADAU17X1_MICBIAS, micbias << 2);
...
int adau17x1_load_firmware(struct snd_soc_codec *codec, const char *firmware)
{
	int ret;
	int dspsr;

	dspsr = snd_soc_read(codec, ADAU17X1_DSP_SAMPLING_RATE);

	snd_soc_write(codec, ADAU17X1_DSP_ENABLE, 1);
	snd_soc_write(codec, ADAU17X1_DSP_SAMPLING_RATE, 0xf);

	ret = process_sigma_firmware_regmap(codec->dev, codec->control_data,
		firmware);
	if (ret) {
		snd_soc_write(codec, ADAU17X1_DSP_ENABLE, 0);
		return ret;
	}
	snd_soc_write(codec, ADAU17X1_DSP_SAMPLING_RATE, dspsr);
...
int adau17x1_probe(struct snd_soc_codec *codec)
{
	...
	if (ret)
		return ret;
	ret = snd_soc_dapm_new_controls(&codec->dapm, adau17x1_dsp_dapm_widgets,
		ARRAY_SIZE(adau17x1_dsp_dapm_widgets));
	if (ret)
		return ret;
	ret = snd_soc_dapm_add_routes(&codec->dapm, adau17x1_dsp_dapm_routes,
	...
	enum adau17x1_type type, enum snd_soc_control_type control_type)
{
	...
	adau = kzalloc(sizeof(*adau), GFP_KERNEL);
	if (!adau) {
		ret = -ENOMEM;
...
void adau17x1_bus_remove(struct device *dev)
{
	struct adau *adau = dev_get_drvdata(dev);

	regmap_exit(adau->regmap);
	kfree(adau);
}
EXPORT_SYMBOL_GPL(adau17x1_bus_remove);
